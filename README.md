...
You paste a company URL. The pipeline fetches the homepage (Cheerio, with a Cloudflare Browser Run fallback for JS-only sites), `/about` and `/team`, Tavily for citable news snippets, and Google News RSS (locale-aware) for press in the company's region. Claude reads the bundle and emits structured JSON where every fact carries a `supporting_quote`. A post-validator drops any field whose quote isn't literally present in a returned citation, and any contact whose name isn't literally in the quoted span. The UI labels each field `verified` (cited + entity-confirmed), `inferred` (synthesized across sources), or `unknown` (sources didn't support it).
...
1. **An eval harness** (`eval/`) — the brief says the "how confident are you?" question matters most; an eval is the shipped answer. 10 hand-verified companies across big-US, mid-EU, small-BE, and obscure tiers. Numbers, not claims.
...
```bash
cp .env.example .env
# fill in ANTHROPIC_API_KEY (required), TAVILY_API_KEY (required for news),
# CLOUDFLARE_ACCOUNT_ID + CLOUDFLARE_API_TOKEN (optional, for JS-only sites)
...
1. **Citation API grounding** — no citation, drop the field.
...
- **Eval at 10 companies** is too small for real precision/recall. Production wants 50–100+.
